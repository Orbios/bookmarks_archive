...
import {mainProcessMessage} from './ipcMain/senders';

const ElectronStore = require('electron-store');
ElectronStore.initRenderer();

const OPEN_DEV_TOOLS_ON_FIRST_LOAD = false;
...
async function createWindow() {
  ...
  if (process.env.VITE_DEV_SERVER_URL) {
    ...
    if (OPEN_DEV_TOOLS_ON_FIRST_LOAD) win.webContents.openDevTools();
  } else {
...
(async (): Promise<void> => {
  ...
  // Set up messaging between main and renderer processes, and application menu
  buildMenu();
  initIpcListeners();
})();
